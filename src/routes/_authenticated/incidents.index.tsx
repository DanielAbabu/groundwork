...
export const Route = createFileRoute("/_authenticated/incidents/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Every scenario in the rotation with difficulty, type, your attempts, and whether you resolved it." },
...
function Board() {
  ...
  const [statusFilter, setStatusFilter] = useState<"ALL" | "resolved" | "attempted" | "unattempted">("ALL");
  ...
  const filtered = useMemo(() => {
    ...
    return scenarios
      ...
      .sort((a, b) => {
        ...
        if (sortKey === "difficulty") cmp = DIFFICULTY_ORDER[a.difficulty] - DIFFICULTY_ORDER[b.difficulty];
        if (sortKey === "status")     cmp = (STATUS_ORDER[a._status] ?? 2) - (STATUS_ORDER[b._status] ?? 2);
  ...
  const SEV_DOT: Record<string, string> = {
    "SEV-1": "bg-sev1", "SEV-2": "bg-sev2", "SEV-3": "bg-sev3",
  };
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button
              ...
              className="run-btn flex items-center gap-1.5"
            >
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-1.5">
              <span className="font-mono text-xs text-muted-foreground">{resolved} / {scenarios.length} resolved</span>
      ...
      <div className="border-b border-border bg-background px-6 py-3">
        <div className="mx-auto max-w-6xl flex flex-wrap items-center gap-3">
          ...
          <div className="flex min-w-[200px] flex-1 items-center gap-2 rounded border border-border bg-card px-3 py-1.5">
            <svg className="size-3.5 shrink-0 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-1.5 font-mono text-xs">
            <span className="text-muted-foreground text-[10px] uppercase tracking-wider">Diff:</span>
            {(["ALL", "starter", "routine", "tricky"] as const).map((d) => (
              <button
                key={d}
                onClick={() => setDiffFilter(d)}
                className={`rounded px-2 py-1 transition-colors ${
                  diffFilter === d ? "bg-primary text-white font-semibold" : "text-muted-foreground hover:text-foreground"
                }`}
              >
            ...
            {(["ALL", "resolved", "attempted", "unattempted"] as const).map((s) => (
              <button
                ...
                className={`rounded px-2 py-1 capitalize transition-colors ${
                  statusFilter === s ? "bg-primary text-white font-semibold" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {s === "ALL" ? "All" : s}
              </button>
        ...
        <div className="mx-auto max-w-6xl px-6 py-4">
            ...
            <tbody>
              {filtered.map((scenario) => {
                ...
                return (
                  <tr
                    key={scenario.id}
                    onClick={() => navigate({ to: "/incidents/$slug", params: { slug: scenario.id } })}
                    className="group cursor-pointer border-b border-border/50 hover:bg-[#1c1c1c] transition-colors"
                  >
                    {/* # */}
                    <td className="py-3 pr-4 font-mono text-xs text-muted-foreground">{scenario._index}</td>
                    ...
                    <td className="py-3 pr-4 hidden sm:table-cell">
                      <span className="rounded border border-border bg-card px-2 py-0.5 font-mono text-[10px] text-muted-foreground">
                        {scenario.type}: {TYPE_LABELS[scenario.type as ScenarioType]?.split(":")[0] ?? scenario.type}
                      </span>
                    </td>
          ...
          {filtered.length === 0 && (
            <div className="py-16 text-center">
              <p className="font-mono text-sm text-muted-foreground">No scenarios match your filters.</p>
              <button onClick={() => { setSearch(""); setTypeFilter("ALL"); setDiffFilter("ALL"); setStatusFilter("ALL"); }}
