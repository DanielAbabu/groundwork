...
export function ConsolePanel({ result, running, signal, conceptNote, failedRuns }: ConsolePanelProps) {
  ...
  return (
    <div className="flex h-full flex-col bg-sidebar border-l border-border">
            ...
            {t === "console" && result && (
              <span
                className={`rounded px-1 py-0.5 text-[10px] font-bold ${
                  allPassed ? "bg-pass/20 text-pass" : cases.length > 0 ? "bg-fail/20 text-fail" : "bg-muted text-muted-foreground"
                }`}
              >
      ...
      <div className="flex-1 overflow-y-auto">
        {tab === "signal" && (
          <div className="p-4 slide-up">{signal}</div>
        )}
          ...
          <div className="flex flex-col gap-0 slide-up">
            ...
            {result?.kind === "crash" && (
              <div className="border-b border-border px-4 py-3">
                <p className="font-mono text-xs font-semibold uppercase tracking-wider text-fail">Sandbox Crash</p>
                <pre className="mt-2 overflow-x-auto rounded bg-fail/5 p-3 font-mono text-xs text-fail/80">{result.error}</pre>
              </div>
            )}
            ...
            {result?.kind === "timeout" && (
              <div className="border-b border-border px-4 py-3">
                <p className="font-mono text-xs font-semibold uppercase tracking-wider text-warning">Time Limit Exceeded</p>
                <p className="mt-1 text-sm text-muted-foreground">Execution timed out after 3 000 ms.</p>
              </div>
            )}
            ...
            {allPassed && (
              <div className="accepted-flash flex items-center gap-2 border-b border-pass/20 px-4 py-3">
                <svg className="size-5 text-pass" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7" />
                </svg>
                <span className="font-mono text-sm font-bold text-pass">Accepted — All tests pass</span>
              </div>
            )}
            ...
            {cases.length > 0 && (
              <div className="flex items-center justify-between border-b border-border px-4 py-2">
                ...
                <span className={`font-mono text-sm font-bold ${allPassed ? "text-pass" : "text-fail"}`}>
            ...
            {cases.map((c, i) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <span className={`mt-0.5 shrink-0 font-mono text-base font-bold ${c.passed ? "text-pass" : "text-fail"}`}>
                    {c.passed ? "✓" : "✗"}
                  </span>
                  ...
                  <span className={`shrink-0 font-mono text-xs font-semibold ${c.passed ? "text-pass" : "text-fail"}`}>
                    {c.passed ? "PASS" : "FAIL"}
                  </span>
                </div>
              </div>
              ...
              <div className="border-t border-border">
                ...
                {conceptOpen && (
                  <div className="slide-up space-y-4 border-t border-border bg-primary/5 px-4 py-4">
                    <div>
                      <p className="font-mono text-[10px] uppercase tracking-widest text-primary">Concept</p>
                      <p className="mt-1 font-semibold text-foreground">{conceptNote.concept}</p>
                    </div>
                    <div>
                      <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Explanation</p>
                      <p className="mt-1 text-sm leading-relaxed text-foreground">{conceptNote.explanation}</p>
                    </div>
                    <div>
                      <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Real-World Analogy</p>
                      <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{conceptNote.realWorldAnalogy}</p>
                    </div>
                    <div>
                      <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Fix Pattern</p>
                      <pre className="mt-1 overflow-x-auto rounded bg-card px-3 py-2 font-mono text-xs text-foreground">{conceptNote.fixPattern}</pre>
                    </div>
                  </div>
                )}
              </div>
            ...
            {!running && !result && (
              <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
                <svg className="size-8 text-border" fill="none" stroke="currentColor" viewBox="0 0 24 24">
